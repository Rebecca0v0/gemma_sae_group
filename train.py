...
def make_torch_comms(n_op_shards=3, n_replicas=1):
...
def training_loop_(
    ...
    for i, (x_tensor, group_vector, group_label) in enumerate(train_acts_iter):
        x_tensor = x_tensor.cuda()
        group_vector = group_vector.cuda()
        group_label = group_label.cuda()
        
        x_full = torch.cat([x_tensor, group_vector], dim=-1)
